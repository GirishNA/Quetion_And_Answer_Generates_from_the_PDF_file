...
import distractorgeneration.CurrentDate;
...
import distractorgeneration.PDFToText;
import distractorgeneration.PDFToTextWithHeadersDelemeter;
import distractorgeneration.TextToJsonConvert;
...
public class QuestionAsker {
	...
	public static File PDF_FILE_PATH;
	 ...
	 */
		...
		try{
			//while(true){
				...
				//String query = "file: "+ Configuration.INPUT_FILE_PATH+Configuration.INPUT_FILE_NAME+" output.txt";
				String query = "file: "+ Configuration.INPUT_FILE_PATH+" output.txt";
				...
				if (query.startsWith("FILE: ")||query.startsWith("file: ")) {
					System.out.println("----------------->");
						...
						else
						{
							...
							File listOfFiles = new File(files[0]);
							for(File eachFile: listOfFiles.listFiles()) {
								if(eachFile.getName().endsWith(".pdf")) {
									System.out.println("pdf file "+eachFile.getName());
									//PDFToText pdf=new PDFToText();
									PDF_FILE_PATH = eachFile;
									PDFToTextWithHeadersDelemeter.convertPDFToText(eachFile);
									INPUT_FILE_NAME = Configuration.INPUT_TEXT_FILE_NAME;
									//INPUT_FILE_NAME = "11.txt";
								}
							}
							in = new FileReader(Configuration.INPUT_FILE_PATH+INPUT_FILE_NAME);
						}
			...
			while(true){
				...
				if(firstLine == 0 && !buf.matches("^[(A-Z)|\\s]*$")) {
					fw.write("FIRST LINE HEADER IN FILE \n");
					firstLine=1;
				}
				...
				System.out.println("------first line data--------->"+buf);
				
				while(flag && buf != null) {
					 if(buf.matches("^[(A-Z)|\\s]*$")) {
					//	System.out.println("+++++++++++++++++++11111111+++++++++++++++++++++++++++++"+buf);
						fw.write(buf+"\n");		
						buf = br.readLine();
					}else if(buf.equals(Configuration.DELIMITERS)) {
						//fw.write("\n"+buf+"\n"+br.readLine()+"\n");
						localDelemeters = buf+"\n" ;//+br.readLine()+"\n" ;
						buf = br.readLine();
						if(buf != null) localDelemeters +=buf+"\n";
						doc = doc.replaceAll("[^A-Za-z0-9. ]+", "");
						firstLine=1;
						flag = false;
				//		System.out.println("+++++++++++++++++++++22222222+++++++++++++++++++++++++++"+buf);

					}else if(buf.matches("^.*\\S.*$")){				
				//		System.out.println("++++++++++++++++++++++33333333++++++++++++++++++++++++++"+buf);
						doc += buf + " ";
						buf = br.readLine();
					}else{
				//		System.out.println("+++++++++++++++++++++++4444444444+++++++++++++++++++++++++"+buf);
						doc += "\n";
						buf = br.readLine();
					}
				}
				...
				/*while(br.ready()){
					...
					if(buf == null){
						break;
					}else if(!buf.equals(Configuration.delemeters)) {
						fw.write("\n"+buf);
						//break;
					}
					if(buf.matches("^.*\\S.*$")){
				...
				System.out.println("------------111111111111111111----------------->"+doc);
				
				Configuration.INPUT_TEXT=doc;
				
				//Configuration.INPUT_TEXT="the following applies : - the staff member must effect the instructions to deal within - hours ; - once clearance has been received the dealing must be executed by the end of the following business day ; and. - if the dealing is not executed by the end of the following business day , the clearance to deal lapses and the staff member must submit a subsequent waiver request to Compliance . ";
						//"we are a company of bold but humble techies who radically accelerate business transformation.";
				
				System.out.println("Saving input text in Configuration.INPUT_TEXT");
				long startTime = System.currentTimeMillis();
				List<String> sentences = AnalysisUtilities.getSentences(doc);
				//List<String> sentences = AnalysisUtilities.getSentences("the following applies : - the staff member must effect the instructions to deal within - hours ; - once clearance has been received the dealing must be executed by the end of the following business day ; and. - if the dealing is not executed by the end of the following business day , the clearance to deal lapses and the staff member must submit a subsequent waiver request to Compliance .");
				System.out.println("Printing Input sentences-"+Configuration.INPUT_TEXT);
			...
			//	if(GlobalProperties.getDebug()) System.err.println("Seconds Elapsed Parsing:\t"+((System.currentTimeMillis()-startTime)/1000.0));
				...
				for(Question question: outputQuestionList){
					...
					fw.write("Q: "+question.yield()+"\n");
					System.out.println("Score :"+question.getScore());
					
					String ansSentence1 = question.getSourceTree().yield().toString();
					fw.write("A: "+ansSentence1+"\n");
					System.out.println("Answer Sentence :"+ansSentence1);
				...
				System.out.println(localDelemeters);
				fw.write(localDelemeters);
			...
			//	VocabularyQuestion.populateTagMap();
				...
				if(Configuration.JSON_CONVERT) {
					TextToJsonConvert.textToJson();
				}
				try {
					Files.move(PDF_FILE_PATH.toPath(), Paths.get(Configuration.FILE_MOVE_PATH+CurrentDate.currentDate()+"_"+PDF_FILE_PATH.getName()));
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
